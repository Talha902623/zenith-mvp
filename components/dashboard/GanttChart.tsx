...
const GanttChart: React.FC<GanttChartProps> = ({ projects, tasks }) => {
    const [selectedProjectId, setSelectedProjectId] = useState<number | null>(projects.find(p => p.status !== 'Completed')?.id || projects[0]?.id || null);
    ...
    if (projects.length === 0) {
        return <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 text-center">No projects available to display.</div>
    }
